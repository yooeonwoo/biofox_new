...
// 🚀 통합 대시보드 API - Xano 백엔드와 연동
export async function GET() {
  try {
    console.log('통합 대시보드 API 요청 시작 (Xano 백엔드)');
    ...
    const currentMonth = getCurrentYearMonth(); // "2025-01"
    const previousMonth = getPreviousMonth(currentDate); // "2024-12"
    ...
    try {
      // 🚀 Xano 백엔드에서 데이터 조회
      const dashboardData = await DashboardAdapter.getDashboardData(
        user.id,
        currentMonth,
        previousMonth
      );

      console.log('Xano 대시보드 데이터 조회 성공');
      ...
      const currentMonthCompact = currentMonth.replace('-', ''); // "202501"
      const previousMonthCompact = previousMonth.replace('-', ''); // "202412"
      ...
      console.log(`통합 대시보드 데이터 생성 완료 (Supabase fallback): KOL ID=${kolData.id}`);
