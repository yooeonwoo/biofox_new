...
type Props = {
  value: { application?: boolean; completion?: boolean; educator?: boolean };
  onToggle: (key: "application" | "completion" | "educator") => void;
  hideIntegratedStar?: boolean;
};

export default function ShopExpertCourseTabs({ value, onToggle, hideIntegratedStar }: Props) {
  const SEG = [
    { key: "application", label: "신청", active: value.application },
    { key: "completion", label: "완료", active: value.completion },
    { key: "educator", label: "교육담당", active: value.educator },
  ] as const;

  const allDone = value.application && value.completion && value.educator;
...
export const getShopExpertCourseStarState = (value: { application?: boolean; completion?: boolean; educator?: boolean }) => {
  return value.application && value.completion && value.educator;
}; 