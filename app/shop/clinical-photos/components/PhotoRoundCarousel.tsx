  ...
  ({
    ...
    const photosByRound = React.useMemo(() => {
      ...
      photos.forEach(photo => {
        ...
        const roundData = rounds[photoRoundDay];
        if (roundData) {
          const slotIndex = roundData.findIndex(slot => slot.angle === photo.angle);
          if (slotIndex !== -1) {
    ...
    const getCompletedRounds = () => {
      ...
      for (const round of roundDays) {
        ...
        const allAnglesUploaded = ['front', 'left_side', 'right_side'].every(angle => {
    ...
    const getNextSlot = () => {
      ...
      for (const round of roundDays) {
        ...
        const angles = ['front', 'left_side', 'right_side'] as const;
    ...
    return (
      <div className="rounded-lg border-2 border-gray-200 bg-gray-50/50 p-2">
        {/* 숨겨진 파일 입력 */}
        <input
          ref={fileInputRef}
          type="file"
        ...
        <div className="mb-2 flex justify-between">
          <div className="flex-1 text-center">
            <h3 className="text-xs font-medium text-gray-700">{getRoundName(currentRound)}</h3>
          </div>
          {!isCompleted && visibleSlots[3] && (
            <div style={{ width: 'calc(25% - 8px)' }} className="text-center">
              <h3 className="text-xs font-medium text-gray-400">
                {getRoundName(visibleSlots[3].roundDay ?? 1)} {/* ✅ 기본값 추가 */}
              </h3>
            </div>
          )}
        </div>
        ...
        <div className="relative flex items-start gap-2">
          ...
          {canGoPrev && (
            <Button
              variant="default"
              size="sm"
              className="legacy-btn absolute left-0 top-1/2 z-10 h-8 w-8 -translate-y-1/2 transform p-0 shadow-xl hover:scale-110"
            ...
            <div className="col-span-3 rounded-lg border-2 border-gray-300 bg-white p-1">
              <div className="grid h-full grid-cols-3 gap-1">
                {visibleSlots.slice(0, 3).map(slot => {
                  ...
                  const isEmptySlot = (slot.roundDay ?? 1) === 0; // 빈 슬롯 - 기본값 적용
          ...
          {canGoNext && (
            <Button
              variant="default"
              size="sm"
              className="legacy-btn absolute right-0 top-1/2 z-10 h-8 w-8 -translate-y-1/2 transform p-0 shadow-xl hover:scale-110"
